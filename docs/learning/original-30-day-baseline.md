...
本文件是“学什么”的唯一冻结依据；实际学习节奏和技术深度见 [8 周核心执行计划](eight-week-core-plan.md)。该执行计划只能补充时间安排、必会/提升边界和验收方式，不能更改本文件的主题、顺序、验收核心或不可改变的原约束。

每次学习从 [学习首页](README.md) 进入，再依次阅读执行计划、当前 `day-*.md` 讲义、[补课台账](learning-gap-ledger.md) 和 [Java 面试 PPT 覆盖地图](java-interview-ppt-map.md)。
